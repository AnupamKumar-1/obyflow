...
export class OpenAILLMAdapter implements LLMAdapter {
  ...
  constructor(config: LLMAdapterConfig = {}) {
    const apiKey = resolveConfigValue(config.apiKey, "OPENAI_API_KEY");
    if (!apiKey) {
      throw new LLMConfigError(
        "Missing OpenAI API key. Pass apiKey to OpenAILLMAdapter or set OPENAI_API_KEY.",
      );
    }
    ...
    this.maxTokens =
      ...
      DEFAULT_MAX_TOKENS;
    this.temperature =
      resolveNumberConfigValue(config.temperature, "OBYFLOW_OPENAI_TEMPERATURE") ??
      DEFAULT_TEMPERATURE;

    const baseURL = resolveConfigValue(config.baseUrl, "OBYFLOW_OPENAI_BASE_URL");
  ...
  async investigate(
    ...
    const toolCall = message?.tool_calls?.find(
      (call): call is OpenAI.Chat.Completions.ChatCompletionMessageFunctionToolCall =>
        call.type === "function" && call.function.name === FINDING_TOOL_NAME,
    );

    if (!toolCall) {
      throw new Error(
        "OpenAI response did not include a tool call for submit_investigation_finding",
      );
    }

    let finding: RawFinding;
    try {
      finding = JSON.parse(toolCall.function.arguments) as RawFinding;
    } catch {
    ...
    if (!isValidFinding(finding)) {
      throw new Error(
        "OpenAI tool call arguments did not match the expected investigation finding shape",
      );
    }
    ...
    return {
      ...
      raw_response: JSON.stringify(message ?? {}),
      usage,
